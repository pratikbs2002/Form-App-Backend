...
public interface FillFormRepository extends JpaRepository<FillForm, Long> {
  ...
  @Transactional
  @Modifying
  @Query(value = "INSERT INTO fill_form (form_id, user_id, answers, created_at, location, location_id) VALUES (:formId, :userId, CAST(:answers AS jsonb), :createdAt, ROW(:lat, :lng)::loc, :locationId)", nativeQuery = true)
  void insertFillForm(Long formId, Long userId, String answers, LocalDateTime createdAt, double lat, double lng,
      Integer locationId);
  ...
  void updateFillForm(Long formId, Long userId, String answers, LocalDateTime createdAt, LocationType location,
      Integer locationId);
